...
export const notifyPackageExpiringSoon = async ({ userId, subscription, pkg, daysLeft }) => {
  return safe(() => createNotification({
    ...
    metadata: {
      ...
      packageName: pkg?.name || subscription.packageCode,
...
export const notifyPackageExpired = async ({ userId, subscription, pkg }) => {
  return safe(() => createNotification({
    ...
    content: `Gói "${pkg?.name || subscription.packageCode}" của bạn đã hết hạn vào ${formatDate(subscription.expiredAt)}. Mua gói mới để tiếp tục sử dụng dịch vụ.`,
    channels: [NotificationChannel.IN_APP, NotificationChannel.EMAIL],
    metadata: {
      subscriptionId: subscription._id?.toString?.() ?? String(subscription._id),
      packageId: subscription.packageId?.toString?.() ?? String(subscription.packageId),
